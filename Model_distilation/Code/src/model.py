...
class SpliceFormer(nn.Module):
    ...
    def forward(self, features, **kwargs): #UPDATE HERE
        ...
        logits = self.conv_final(skip) #UPDATE HERE
        out = m_1(logits)

        if kwargs.get('distillation_mode', False):
            #UPDATE HERE - RETURNING LOGITS
            return logits, acceptor_actions, donor_actions, acceptor_log_probs, donor_log_probs
        
        if self.crop:
            out = out[:,:,(self.CL_max//2):-(self.CL_max//2)]
            logits = logits[:,:,(self.CL_max//2):-(self.CL_max//2)] #UPDATE HERE
        if self.returnFmap:
...
#-----lITE VERSION (SPLICEAI)- UPDATE HERE-------

class SpliceAILite(nn.Module):
    def __init__(self, CL_max, bn_momentum=0.01, lite_channels=16, lite_blocks=2, **kwargs):
        super().__init__()
        self.n_channels = lite_channels #32 in the original one! - UPDATE HERE
        self.CL_max = CL_max #UPDATE HERE
       
       #We reduced the layers to 2 and used the corresponding dilations - UPDATE HERE
        self.res_W = [11, 11] #[11, 11, 21, 41]
        res_dilation = [1, 4] #[1, 4, 10, 25]
        
        self.kernel_size = 1
        self.conv_layer_1 = nn.Conv1d(in_channels=4, out_channels=self.n_channels, kernel_size=self.kernel_size, stride=1)
        
        # The skip layers needs to be adjusted to the new quantity of blocks + 1
        self.skip_layers = nn.ModuleList([nn.Conv1d(in_channels=self.n_channels, out_channels=self.n_channels, kernel_size=self.kernel_size, stride=1) for i in range(lite_blocks + 1)])
        self.res_layers = nn.ModuleList([ResComboBlock(in_channels=self.n_channels, out_channels=self.n_channels, res_W=self.res_W[i], res_dilation=res_dilation[i], bn_momentum=bn_momentum) for i in range(lite_blocks)])
...
#-------STUDENT MODEL(SPLICEFORMERLITE) - UPDATE HERE--------

class SpliceFormerLite(nn.Module):
    def __init__(self, CL_max, n_channels=16, maxSeqLength=4*128, depth=2, n_transformer_blocks=1, heads=2, dim_head=32, mlp_dim=256, dropout=0.01, returnFmap=False, bn_momentum=0.01, determenistic=False, crop=True, **kwargs):
        super().__init__()
        self.n_channels = n_channels #Reduced to 16 (ou teste 8/12)
        self.CL_max = CL_max
        self.kernel_size = 1
        self.crop = crop
        self.returnFmap = returnFmap
        
        #Using lite version of CNN
        self.SpliceAI = SpliceAILite(CL_max, bn_momentum=bn_momentum, lite_channels=n_channels, lite_blocks=2).apply(keras_init)
        
        self.conv_final = nn.Conv1d(in_channels=self.n_channels, out_channels=3, kernel_size=self.kernel_size, stride=1)
        self.maxSeqLength = maxSeqLength # KEEP 512
        self.policy = Policy(n_channels=n_channels)
        self.determenistic = determenistic
    ...
    def forward(self, features):
        ...
        #WE GONNA NEED THE LOGITS BEFORE THE SOFTMAX TO DISTILATION
        logits = self.conv_final(skip)
        out = m_1(logits) 

        if self.crop:
            out = out[:,:,(self.CL_max//2):-(self.CL_max//2)]
            logits = logits[:,:,(self.CL_max//2):-(self.CL_max//2)]
        if self.returnFmap:
            #Trying to simplify
            pass 
        else:
            # Returning logits to use on soft loss and out to validate
            return logits, acceptor_actions, donor_actions, acceptor_log_probs, donor_log_probs
