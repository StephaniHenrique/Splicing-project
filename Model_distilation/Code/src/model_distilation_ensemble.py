...
teacher_models = []
print("Carregando Ensemble de Professores...")
...
try:
   for i, path in enumerate(TEACHER_PATHS):
        ...
        print(f"  Teacher {i+1} carregado de {path.split('/')[-1]}")
...
# --- NOVO: Ponderação de Classes ---
# Pesos Inversos: W_Null, W_Acceptor, W_Donor. 
# Null deve ter peso baixo (1) e Acceptor/Donor peso alto (ex: 10000)
# A densidade de sítios é muito baixa.
...
for epoch in range(1, EPOCHS + 1):
    ...
    for i, (input_data, hard_targets) in enumerate(train_loop):
        ...
        # 3. Alinhamento e Corte dos Logits do Professor e Hard targetd
        L_TARGET = student_logits.shape[2] 
...
#   # --- Validação ---
    ...
    with torch.no_grad():
        ...
        for input_data, hard_targets in val_loop:
            ...
            # O hard_loss_fn agora é ponderado.
    ...
    # REMOVIDO: val_metrics = print_topl_statistics(val_logits_list, val_targets_list) # Causava o ValueError
...
#  acceptor_metrics = print_topl_statistics(y_true_acceptor, y_pred_acceptor)
    ...
    # REMOVIDO: print(y_true_donor) e print(y_pred_donor)
